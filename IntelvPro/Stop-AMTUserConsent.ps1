...
<#
  ...
  .Description
    This CmdLet stops the user consent process on clients that have Intel Active Management Technology (AMT) firmware version 6.0 and higher.
  .Notes
    Supported AMT Firmware Versions: 6.0 and higher.
  ...
  .Example
    Stop-AMTUserConsent vproclient.vprodemo.com -credential $AMTCredential
		...
		ForEach ($Comp in $ComputerName) {
			...
			try {
				...
				if ($IPSObj.GetProperty("OptInRequired").ToString() -ne 0) {
					...
					#Check user consent started
					if ($IPSObj.GetProperty("OptInState").ToString() -ne 0) {

						#Stop user consent process
						$InputObj = $ObjRef.CreateMethodInput("CancelOptIn")
						...
						} while ($IPSObj.GetProperty("OptInState").ToString() -ne 0)

					} else {
						$ReturnValue = 0
					}
				} else {
					#Skip stop user consent operation
					$ReturnValue = 0			
				}
				...
				if ($_.Exception.GetType().ToString() -eq "Intel.Management.Wsman.WsmanUnreachableException") {
					...
					$Obj | Add-Member -MemberType noteproperty -Name ComputerName -value $Comp
					$Obj | Add-Member -MemberType noteproperty -Name Status -value "Not Required"
				} 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUb5CFo2edYHogxeb0Q4uLD8Cx
# olegghW+MIIDPTCCAqagAwIBAgIDBbD/MA0GCSqGSIb3DQEBBQUAME4xCzAJBgNV
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUk7U8N1x3e5+GMURvf6YluNB2
# s+kwDQYJKoZIhvcNAQEBBQAEgYBWGdLjGwMwQqlyEEM+TyGSuMOiMsOGO+ZdQM4E
# WsPury1K9XOScH7176f29PuPCnmZdMYzn9MTQUhpAAi3kbuLsS7OSMQTXzBIxjGE
# pxy2s4rZ3PWEliVRAca703rvWJgvJ0eZyQXFe1pk3EGbVScUOvIBFR6nK/pmwkBo
# n0y40aGCAX8wggF7BgkqhkiG9w0BCQYxggFsMIIBaAIBATBnMFMxCzAJBgNVBAYT
...
# DxcNMTIwMTE5MjMxNzM4WjAjBgkqhkiG9w0BCQQxFgQU9Qi9uhHJ3EeHh68Zn7+T
# CXlLATYwDQYJKoZIhvcNAQEBBQAEgYCAM5UrNTL+8719gQuPcxcguFqlj0CvPwWX
# akmxH1pp1vdp4sgNbsbO7Z+MmIC+4On5R2zbURomYZasN7ny4eDnLzsS9SZqvjVN
# MIvligS/JkoXcTJ5vNzC/MbIgbN5yXdac2EmxfsEBknTXeeo5IkLY9JjAIKErCcY
# aOZDAgP7Lw==
