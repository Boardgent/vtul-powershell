function ParseInterval
	{
		...
		return "$DayPart days, $HourPart hours, $MinPart minutes, $SecPart seconds"
	}
	...
	function ParseDateString
	{
		...
		return [System.DateTime]::Parse($args[0]).ToString("F")
	}
...
PROCESS 
{
		...
		Foreach ($comp in $ComputerName)
		{
			...
			if($temp[0] -lt "8")
			{
				...
				{  			
					$NextAlarm = ParseDateString $AlarmClockService.GetProperty("NextAMTAlarmTime").Object.Text $GMTError
				}
				...
				{
					$AlarmInterval = ParseInterval $AlarmClockService.GetProperty("AMTAlarmClockInterval").Object.Text
				}
			...
			else
			{
				...
				{
					if ($AMTAlarmClock.Object.GetProperty("Interval").IsNull)
					{
						$AlarmInterval = "[None Set]"
					}
					else
					{
						$AlarmInterval = ParseInterval $AMTAlarmClock.Object.GetProperty("Interval").Object.Text
					}
					if($AMTAlarmClock.Object.GetProperty("StartTime").IsNull)
					{
						$NextAlarm = "[None Set]"
					}
					else
					{
						$NextAlarm = ParseDateString $AMTAlarmClock.Object.GetProperty("StartTime").Object.Text $GMTError
					}
					
					if($AMTAlarmClock.Object.GetProperty("DeleteOnCompletion").ToString() -eq "true")
					{
						$DeleteOption = "AutoDeleteOnCompletion" 
					}
					else
					{
						$DeleteOption = "ManualDelete"
					}
					$AlarmName = $AMTAlarmClock.Object.GetProperty("InstanceID").ToString()
					$Obj | Add-Member -MemberType noteproperty -Name "AlarmName" -value $AlarmName
					$Obj | Add-Member -MemberType noteproperty -Name "NextAlarmTime" -value $NextAlarm
					$Obj | Add-Member -MemberType noteproperty -Name "PeriodicInterval" -value $AlarmInterval
					$Obj | Add-Member -MemberType noteproperty -Name "DeleteOption" -value $DeleteOption
					$Obj | Add-Member -MemberType noteproperty -Name "Status" -value "Successful"
					$Results += $Obj 
					Write-Output $Results
					$Obj = new-object psobject
