...
<#
  ...
  .Notes
    ...
    If no ID is specified the oldest IDER session is closed.
  ...
  .Link
    ...
    .Example
    Stop-AMTIDER #SessionID
    .Example
    Stop-AMTIDER
    
    If no ID is specified the oldest IDER session is closed. 
    .Example
    Stop-AMTIDER -CloseAllSessions
...
[CmdletBinding()]
Param (
[Parameter(Mandatory=$false,ValueFromPipelineByPropertyName=$true,ValueFromPipeline=$false, position=0, HelpMessage="Enter valid session ID")] [String[]] $IDERID,
[Parameter(Mandatory=$false,ValueFromPipelineByPropertyName=$true,ValueFromPipeline=$false, HelpMessage="Close all IDER sessions")] [switch] $CloseAllSessions
)

PROCESS {
    if ($CloseAllSessions.IsPresent)
    {
        if($global:imrLib) 
        {
            $imrResult = $global:imrLib.Close()
            $numberSessions = $AMTIDERSessions.Count
            if($AMTIDERSessions.Count -gt 0)
            {
                For($i=1; $i -le $numberSessions; $i++)
                {          
                    $global:AMTIDERSessions.RemoveAt(0)
                }
            }
            write-output "All IDER sessions closed"
        }            
        return;
    }
    ...
    Write-Output "Thegfgdfgdgdfions"
    $Results = @()
  
    ForEach ($id in $IDERID) {
	   $Obj = new-object psobject
       [int]$count = 0
       ForEach ($s in $AMTIDERSessions)
       {
         if ($s.IDERSessionID -eq $id)
         {
           $session = $s
           $cur = $count
         }
       ...
       try {
	       #close the session
           $imrResult = $global:imrLib.IDERCloseSession($session.IDERSessionID)
           $imrResult = $global:imrLib.RemoveClient($session.IDERSessionID)
           if($global:AMTIDERSessions.Count -gt 0)
           {
                $global:AMTIDERSessions.RemoveAt($cur)
           }
           ...
           { 
                 if($id -ne $null)
                {
                    $Obj | Add-Member -MemberType noteproperty -Name ID -value $session.IDERSessionID 
                }
                else
                {
                    $Obj | Add-Member -MemberType noteproperty -Name ID -value "oldest IDER session"
                }
           }
           else
           { 
		   ...
		   $Obj | Add-Member -MemberType noteproperty -Name ID -value $session.IDERSessionID 
           $Obj | Add-Member -MemberType noteproperty -Name Status -value "Stop Failed"
		}

	   #Append to Result 
	   $Results += $Obj
       Write-Output $Obj
       if (($AMTIDERSessions.Count) -eq 0)
        {
            Write-Output "There are no open IDER sessions"
            $imrResult = $global:imrLib.Close()          
        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUXfFuygmn1HVmkscj/U0557QI
# ERCgghW+MIIDPTCCAqagAwIBAgIDBbD/MA0GCSqGSIb3DQEBBQUAME4xCzAJBgNV
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUz3rEHUani5yTWFdwiph+XxeD
# jS0wDQYJKoZIhvcNAQEBBQAEgYA3P++8g+nQk0bTMctHJLS8yktk9xAoajzp5A0r
# fCEgnQdckhlV1DWPIuSSR22cX7mCs3NCyCGAz/Of70LmeuXqLpuQGxCmZwOvIqjp
# N6LHcl2QyYh3UkiVwwXwdw/sEfI08/XIRHDZlwLD+tcUeYEIg1gv93ar5W//d1hN
# P/eN9qGCAX8wggF7BgkqhkiG9w0BCQYxggFsMIIBaAIBATBnMFMxCzAJBgNVBAYT
...
# DxcNMTIwMTE5MjMxNzM3WjAjBgkqhkiG9w0BCQQxFgQU5OFKxmq9J7ayEHKyZ1bu
# 5SBpYNAwDQYJKoZIhvcNAQEBBQAEgYAT7x6MXFRhkj7g8LuwNG/Lt1dmV4jntuBQ
# 6mdDKqEbdvrHPWIDKzut/ErnkYrTwbyeOVV469nBXSNPyvMa0gsuaFcTWYSiWYwk
# MYt4RXE2Q6zEcW85dkt8Em9pPfGTU+AOfVVerWbkGm9gIHfwH1Ak22dDLrVPUebk
# mvC/XRoYug==
