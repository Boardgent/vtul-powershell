...
Foreach ($comp in $ComputerName)
{
  $Connection.SetHost($comp, $port)
    ...
    while($en.HasNext)
    {
        ...
        {
                 if($NetworkPortSystemDefensePolicy.GetProperty("Enabled").ToString() -eq "true")
                {
                    #
        ...
        $FilterObject = $Filter_EPR.Get()
        if($FilterObject.GetProperty("FilterProfile").ToString() -eq "1")
        {
            # Filter Profile is Drop
            if($FilterObject.GetProperty("HdrProtocolID8021").ToString() -eq "2048")
            {
                #Header Protocol is IP
                if($NetworkPortSystemDefensePolicy.GetProperty("Enabled").ToString() -eq "true")
                {
                    #
                    # System Defense is Active
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVxYZUKQDI2b9if1hcyWP8Syz
# 2cOgghW+MIIDPTCCAqagAwIBAgIDBbD/MA0GCSqGSIb3DQEBBQUAME4xCzAJBgNV
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUdHrKWX8R/689hAN/jK1RmHi7
# 39QwDQYJKoZIhvcNAQEBBQAEgYA4DA7ddQlxMnas+xy8OZxvHRC8/0l0ilVIjFxU
# u/01/Xj8PzAnsqPs8U8PkW+MsDTIaDzzpA4F2olVy83wTkMFHtD0bppl4/3NHNnn
# a0pE8Ekhloxtb0dSPrEugZyA6PdIni6/60WQyY+LHbG7iZ8PIRCLoXsJg9RR4alU
# X9lYHqGCAX8wggF7BgkqhkiG9w0BCQYxggFsMIIBaAIBATBnMFMxCzAJBgNVBAYT
...
# DxcNMTIwMTE5MjMxNzIyWjAjBgkqhkiG9w0BCQQxFgQU2buqUZRwefMYxVvzQfaY
# BF4GxSswDQYJKoZIhvcNAQEBBQAEgYClIo3MICXJxxmUwDCTENeWfCSymWzyQdt4
# PQmlQf5MAsKoBv8dXc5GjGqvDeymksz34SqmAIJfZGygWtoZCFAjUmVkstIhuUCo
# AZUl/H00Pi/NIGcAd4wq/HxEuRhcxh2vNrvsG3R7Evt5+Mmgwc/R/A0cLTajA+gf
# ufG5Q5ppzw==
