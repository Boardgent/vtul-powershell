...
<#
  ...
  .Description
    This CmdLet clears the Intel Active Management Technology (AMT) alarm clock timers from clients that have Intel AMT firmware version 5.1 or higher. 
	From AMT firmaware version 8.0 or higher, user can supply a specific alarm name to delete. Otherwise, all alarm timers will be deleted. 
  .Notes
  ...
  .Link
    ...
    .Example
	Clear-AMTAlarmClock -computer:vproclient.vprodemo.com
	...
	Status           : Successful
	 .Example
	Clear-AMTAlarmClock -computer:vproclient.vprodemo.com -AlarmName MyDefaultAlarm
	...
	Status           : Successful
    .Example
	Clear-AMTAlarmClock vproclient -Username:amtuser
	...
	Status           : Successful
    .Example
	Clear-AMTAlarmClock vproclient.vprodemo.com -Username vprodemo\administrator
	...
	Status           : Successful
    .Example
	Clear-AMTAlarmClock -ComputerName:vproclient.vprodemo.com -credential $AMTCredential -TLS
    ...
    .Example
	Clear-AMTAlarmClock -ComputerName computer1.vprodemo.com,doesnotexist.vprodemo.com | Where {$_.Status -eq "Failed"}
    ...
    .Example
	Get-Content computers.txt | Clear-AMTAlarmClock -TLS
...
Param (
	...
	[Parameter(Mandatory=$false,ValueFromPipelineByPropertyName=$true,ValueFromPipeline=$false, HelpMessage="Alarm Name, if supplied, only the specific alarm will be deleted, in use in Intel AMT vresion 8.0 and later.")][string] $AlarmName,
...
Foreach ($comp in $ComputerName)
{
  $Connection.SetHost($comp, $port)
	...
	{
		$Obj | Add-Member -MemberType noteproperty -Name "Status" -value "Get Version Failed"
		$Results += $Obj 
		continue     
	}
	...
	if($temp[0] -lt "8")
	{
		...
		catch
		{
			...
			$Obj | Add-Member -MemberType noteproperty -Name "NextAlarmTime" -value "[None Set]"
			$Obj | Add-Member -MemberType noteproperty -Name "PeriodicInterval" -value "[None Set]"
			$Results += $Obj 
			continue 
		}
		$AlarmClockService.SetProperty("NextAMTAlarmTime",$null)
		$AlarmClockService.SetProperty("AMTAlarmClockInterval", $null)
	...
	{
		$AlarmClockOccurrences = $Connection.NewReference("IPS_AlarmClockOccurrence").Enumerate()
		foreach($AMTAlarmClock in $AlarmClockOccurrences)
		{
			$instanceId = $AMTAlarmClock.Object.GetProperty("InstanceID").ToString()
			$builder = "SELECT * FROM IPS_AlarmClockOccurrence WHERE InstanceID=`'"
			$builder += $instanceId
			$builder += "`'"
			$delete = "false"
			if($AlarmName -eq "")#Delete all alarm because no specific alarm was supplied.
			{
				$delete = "true"
			}
			else
			{
				if($instanceId -eq $AlarmName)
				{
					$delete= "true"
				}
			}
			if($delete -eq "true")
			{
				$alarmClockOccurrenceRef = $Connection.NewReference($builder)
				$alarmClockOccurrenceRef.Delete()
				$Obj | Add-Member -MemberType noteproperty -Name "AlarmName" -value $instanceId
				$Obj | Add-Member -MemberType noteproperty -Name "NextAlarmTime" -value "[None Set]"
				$Obj | Add-Member -MemberType noteproperty -Name "PeriodicInterval" -value "[None Set]"
				$Obj | Add-Member -MemberType noteproperty -Name "DeleteOption" -value "[None Set]"
				$Obj | Add-Member -MemberType noteproperty -Name "Status" -value "Successful"
				$Results += $Obj 
				Write-Output $Results
				$Obj = new-object psobject
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUEYlUDxNhMFOcgKczgdOBhxZM
# LzqgghW+MIIDPTCCAqagAwIBAgIDBbD/MA0GCSqGSIb3DQEBBQUAME4xCzAJBgNV
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUyL41NA/TWFy8XKsS3XRvC63l
# yMQwDQYJKoZIhvcNAQEBBQAEgYA1AHlwLwr9NGoBuMMIsiya2WuDg0wQL/85UP4k
# PBIzP6fPPxA7I3oCxQX6vhDrRDffcSAPP6It/7PjvxlTZTCqOMtgQULby2kniHrS
# 9fUmMvG7saSgnICNMpZFjmpqLzV4PJcMVSUHt5LD3JpwtyD05VHVZDg8ajGKWm3w
# WgLjAqGCAX8wggF7BgkqhkiG9w0BCQYxggFsMIIBaAIBATBnMFMxCzAJBgNVBAYT
...
# DxcNMTIwMTE5MjMxNzE5WjAjBgkqhkiG9w0BCQQxFgQUgf6pD0wu3lhkVRE3WllD
# j+Z7vSUwDQYJKoZIhvcNAQEBBQAEgYC/JYnRYA66Ikbz7HUxOD5skhJz3V10W/x3
# /k7k8lwwWQDQt4wrhtmT2oWbY1o+7FR2tBMJwdz8gkpMmDyyEAJJ9NZBCHQxK2bH
# xNz6kEuM5qozVT/ijkpJ4tDcN2SUofGt/Fgk2+CoMYjJuuwHRLIQrvnMmkzC7wQB
# WzGURIUa3g==
